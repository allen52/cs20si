...
def conv_relu(inputs, filters, k_size, stride, padding, scope_name):
    ...
    '''
    with tf.variable_scope(scope_name, reuse=tf.AUTO_REUSE) as scope:#why variable_scope?
        input_channel = inputs.shape[-1]
        kernel = tf.get_variable('kernel',
                                 [k_size,k_size,input_channel,filters],
                                 initializer = tf.truncated_normal_initializer()
                                 )
        bias = tf.get_variable('biases',
                               [filters],
                                initializer=tf.random_normal_initializer())                          
        conv_l = tf.nn.conv2d(inputs,kernel,strides = [1,stride,stride,1],
                              padding = padding)
        relu_l = tf.nn.relu(conv_l + bias,name =scope.name)
    #############################
    ########## TO DO ############
    #############################
    return relu_l
...
def maxpool(inputs, ksize, stride, padding='VALID', scope_name='pool'):
    ...
    with tf.variable_scope(scope_name, reuse=tf.AUTO_REUSE) as scope:
        max_pool = tf.nn.max_pool(inputs,ksize = [1,ksize,ksize,1],
...
class ConvNet(object):
    ...
    def inference(self):
        ...
        conv1 = conv_relu(inputs = self.img,
                          ...
                          padding = 'SAME',
                          scope_name = 'convnet1')
        max_pool1 = maxpool(inputs = conv1,ksize = 2,stride = 2,scope_name = 'maxpool1')
        conv2 = conv_relu(inputs =max_pool1,
                          filters = 64,
                          k_size = 5,
                          stride = 1,
                          padding = 'SAME',
                          scope_name = 'convnet2')
        max_pool2 = maxpool(inputs = conv2,ksize = 2,stride = 2,scope_name = 'maxpool2')
        
        dim = max_pool2.shape[1]*max_pool2.shape[2]*max_pool2.shape[3]
        max_pool2 = tf.reshape(max_pool2,[-1,dim])
        fc = fully_connected(max_pool2, 1024, 'fc')
